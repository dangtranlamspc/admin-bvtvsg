...
const AddNewRongRieu = () => {
  ...
  useEffect(() => {
  id && getCTGDRongReuDetail(id);
  }, [id]);
  

  const getCTGDRongReuDetail = async (id: string) => {
  try {
    const snap = await getDoc(doc(fs, `rongrieu/${id}`));
    if (snap.exists()) {
  ...
  const handleAddNewCTGDRongRieu = async (values: any) => {
  setIsLoading(true);
  ...
  try {
    ...
    const snap = id
      ? await updateDoc(doc(fs, `rongrieu/${id}`), data)
      : await addDoc(collection(fs, 'rongrieu'), {
          ...data,
          createdAt: Date.now(),
          rate: 0,
        });
        ...
        <div className="col-md-8 offset-md-2">
          <Card title='THÊM MỚI'>
            <Form disabled={isLoading} 
              ...
              form={form}
              layout='vertical'
              onFinish={handleAddNewCTGDRongRieu}
            >
