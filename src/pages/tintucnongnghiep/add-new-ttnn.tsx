...
const AddNewTTNN = () => {
  ...
  useEffect(() => {
  id && getTinTucNongNghiepDetail(id);
  }, [id]);
  

  const getTinTucNongNghiepDetail = async (id: string) => {
  try {
    const snap = await getDoc(doc(fs, `tintucnongnghiep/${id}`));
    if (snap.exists()) {
  ...
  try {
    ...
    const snap = id
      ? await updateDoc(doc(fs, `tintucnongnghiep/${id}`), data)
      : await addDoc(collection(fs, 'tintucnongnghiep'), {
          ...data,
          createdAt: Date.now(),
          rate: 0,
        });
  ...
  return (
    <div>
    <HeadComponent
      title='THÊM MỚI TIN TỨC NÔNG NGHIỆP'
      pageTitle='THÊM MỚI TIN TỨC NÔNG NGHIỆP'
    />
