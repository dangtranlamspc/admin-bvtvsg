...
const AddNewNNDTHoaCayKieng= () => {
    ...
    useEffect(() => {
		id && getHoaCayKiengDetail(id);
	  }, [id]);
    

    const getHoaCayKiengDetail = async (id: string) => {
		try {
			const snap = await getDoc(doc(fs, `hoacaykieng/${id}`));
			if (snap.exists()) {
    ...
    const handleAddNewHoaCayKieng = async (values: any) => {
		setIsLoading(true);
		...
		try {
			...
			const snap = id
				? await updateDoc(doc(fs, `hoacaykieng/${id}`), data)
				: await addDoc(collection(fs, 'hoacaykieng'), {
						...data,
						createdAt: Date.now(),
						rate: 0,
				  });
        ...
        <div className="col-md-8 offset-md-2">
          <Card title='THÊM MỚI'>
            <Form disabled={isLoading} 
              ...
              form={form}
              layout='vertical'
              onFinish={handleAddNewHoaCayKieng}
            >
...
export default AddNewNNDTHoaCayKieng