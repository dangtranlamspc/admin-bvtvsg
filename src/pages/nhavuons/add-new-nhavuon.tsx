...
const AddNewConTrungTrongNhaVuon = () => {
  ...
  useEffect(() => {
  id && getCTGDNhaVuonDetail(id);
  }, [id]);
  

  const getCTGDNhaVuonDetail = async (id: string) => {
  try {
    const snap = await getDoc(doc(fs, `nhavuons/${id}`));
    if (snap.exists()) {
  ...
  const handleAddNewCTGDNhaVuon = async (values: any) => {
  setIsLoading(true);
  ...
  try {
    ...
    const snap = id
      ? await updateDoc(doc(fs, `nhavuons/${id}`), data)
      : await addDoc(collection(fs, 'nhavuons'), {
          ...data,
          createdAt: Date.now(),
          rate: 0,
        });
        ...
        <div className="col-md-8 offset-md-2">
          <Card title='THÊM MỚI'>
            <Form disabled={isLoading} 
              ...
              form={form}
              layout='vertical'
              onFinish={handleAddNewCTGDNhaVuon}
            >
...
export default AddNewConTrungTrongNhaVuon